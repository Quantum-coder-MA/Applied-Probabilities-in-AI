...
def load_cancer_data():
    return pd.read_csv("dataCancer.csv")
# ─────────────────────────────────────────────
# PAGE CONFIG
# ─────────────────────────────────────────────
st.set_page_config(
...
# ═══════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        # Load the actual real dataset
        try:
            df_sports = pd.read_csv("multimodal_sports_injury_dataset.csv")
            # Show the first 10 rows and use container width
            st.dataframe(df_sports.head(10), use_container_width=True)
            
            # Optional: Add a small shape metric
    ...
    with tab2:
        ...
        baseline_data = {
            "Modèle":    ["Decision Tree", "Marbar-RF", "GF-MFTL"],
            "Accuracy":  [0.6586, 0.6401, 0.6600],
            "F1-Score":  [0.3788, 0.0000, 0.4100],
            "Recall":    [0.2892, 0.0000, 0.3300],
            "FPR":       [0.1337, 0.0000, 0.1600],
            "AUC":       [0.6017, 0.6551, 0.6200],
            "Log Loss":  [1.2286, 0.6486, 0.8500],
        ...
        x = np.arange(len(metrics_bar)); w = 0.25
        colors_b = [ACCENT, GREEN, ORANGE]
        ...
        axes[0].set_xticks(x+w); axes[0].set_xticklabels(metrics_bar, fontsize=8)
        axes[0].set_title("Métriques de Classification", fontsize=9)
        axes[0].legend(fontsize=7); axes[0].set_ylim(0, 0.85)
        ...
        axes[1].set_xticks(x2+w); axes[1].set_xticklabels(metrics_bar2, fontsize=8)
...
# ═══════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        # Load the actual real dataset
        try:
            df_cancer = pd.read_csv("dataCancer.csv")
    ...
    with tab2:
        ...
        baseline_cancer = pd.DataFrame({
            "Modèle":       ["Naive Bayes", "KNN (k=9)", "Random Forest"],
            "Accuracy":     [0.9371, 0.9790, 0.9580],
            "F1-Score":     [0.9540, 0.9820, 0.9600],
            "Recall":       [0.9259, 1.0000, 0.9630],
            "FPR":          [0.1304, 0.0566, 0.0435],
            "AUC":          [0.9878, 0.9965, 0.9951],
        })
        ...
        x = np.arange(len(metrics_c)); w = 0.25
        cols_c = [ACCENT, PURPLE, GREEN]
        ...
        ax.set_xticks(x+w); ax.set_xticklabels(metrics_c, fontsize=9)
        ...
        st.info("ℹ️  **KNN (k=9)** obtient un recall parfait (1.0) : aucun cancer manqué. C'est la priorité clinique absolue.")
    ...
    with tab4:
        ...
        bars1 = axes[1].bar(x2 - w/2, avant2, w, label="Avant", color=ACCENT+"88", edgecolor="none")
        bars2 = axes[1].bar(x2 + w/2, apres2, w, label="Après", color=ORANGE+"cc", edgecolor="none")
...
# ═══════════════════════════════════════════════════════════════════
    ...
    # Cancer — Accuracy
    models_c = ["Naive\nBayes", "KNN\n(k=9)", "RF\nAvant", "RF\nAprès"]
    acc_c = [0.9371, 0.9790, 0.9580, 0.9650]
